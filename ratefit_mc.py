...
time=1.0
reaction_rates=[10.*random.random() for i in range(nrates)]
...
mvs=[]
for k in ks:
    mvs.append(IMP.core.NormalMover([k],
                            IMP.FloatKeys([IMP.FloatKey("nuisance")]),
                            maxstep))

mvs.append(IMP.core.NormalMover([sigma],
                            IMP.FloatKeys([IMP.FloatKey("nuisance")]),
                            maxstep))
    ...
    if sf.evaluate(False)<bestscore:
        ...
        for n in range(len(rates)):
            for c in initial_concs:
                ...
                print(tr,reacts[c][n],erra,errb,rest_dict[c][n].unprotected_evaluate(False))
                sumerra+=erra
                nerra+=1
